...
# fig=surface_scatter_plot(Xsim, ysim, lambda x: fitted_tree.predict([x])) #lamdba refers to the "anonymous" lambda funtion
# fig.show() 
...
# DELETE: https://www.youtube.com/watch?v=ok2s1vV9XW0 & #DELETE: https://machinelearningmastery.com/classification-versus-regression-in-machine-learning/
...
for i in np.arange(1, 5): # number of trees in the forest
     ...
     fig=surface_scatter_plot(X_train,y_train,lambda x: model4.predict(X_test))
     fig.show()
...
# DELETE: THIS FUNCTION NEEDS DEBUGGING
...
roc_fig=plot_roc(model5_scaled, X_test, y_test)
plt.show()
...
get_score(model5_scaled, X_train, X_test, y_train, y_test)
